package com.san1ch.vocabanana.core.essentials.database.model.text.exception
...
class TextValidateTooLongException(private val maxSize: Int) : TextException("Text name is too long"){
    override fun getLocalizedMessage(stringProvider: TextStringProvider): String {
        return stringProvider.textNameValidateTooLong(maxSize)
    }
}

class TextValidateNameHasInvalidCharException(private val invalidChar: Char) : TextException("Text name contains invalid char"){
    override fun getLocalizedMessage(stringProvider: TextStringProvider): String {
