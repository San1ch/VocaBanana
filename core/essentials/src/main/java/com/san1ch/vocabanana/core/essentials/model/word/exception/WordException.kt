...
abstract class WordException(
    ...
    ): String = getLocalizedMessage(stringProviderStore<WordStringProvider>())
...
class WordValidateEmptyException : WordException("Word is empty") {
    override fun getLocalizedMessage(stringProvider: WordStringProvider): String = stringProvider.wordEmpty
}

class WordValidateTooLongException(
    private val maxSize: Int,
) : WordException("Word is too long") {
    override fun getLocalizedMessage(
        stringProvider: WordStringProvider,
    ): String = stringProvider.wordTooLong(maxSize)
}
...
class WordValidateInvalidLemmaCharException(
    ...
    override fun getLocalizedMessage(
        stringProvider: WordStringProvider,
    ): String = stringProvider.wordInvalidLemmaChar(invalidChar)
}
