...
abstract class RepositoryException(
    ...
    ): String = getLocalizedMessage(stringProviderStore<RepositoryStringProvider>())
...
class RepositoryNoDataByRequestException : RepositoryException("No data by request") {
    override fun getLocalizedMessage(
        stringProvider: RepositoryStringProvider,
    ): String = stringProvider.thereIsNoDataByRequest
}
